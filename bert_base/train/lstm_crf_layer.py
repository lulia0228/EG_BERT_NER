...
class BLSTM_CRF(object):
    ...
    def add_blstm_crf_layer(self, crf_only):
        ...
        print("lstm_crf_layer.py_line62: ", logits.shape) #(?,128,1)
        # CRF decode, pred_ids 是一条最大概率的标注路径
        pred_ids, _ = crf.crf_decode(potentials=logits, transition_params=trans, sequence_length=self.lengths)
        print("lstm_crf_layer.py_line65: ", pred_ids.shape) #(?,128,1)
    ...
    def project_crf_layer(self, embedding_chars, name=None):
        ...
        with tf.variable_scope("project" if not name else name):
            with tf.variable_scope("logits"):
                print("lstm_crf_layer.py_line142: ", self.num_labels)
                ...
                output = tf.reshape(self.embedded_chars,
                                    shape=[-1, self.embedding_dims])  # [-1, embedding_dims]
                # pred = tf.tanh(tf.nn.xw_plus_b(output, W, b)) # 据说用tanh是严重bug啊，反正关掉tanh，结果好像好一点
                pred = tf.nn.xw_plus_b(output, W, b) # changed by me
                print("lstm_crf_layer.py_line151: ", pred.shape)
            return tf.reshape(pred, [-1, self.seq_length, self.num_labels])
