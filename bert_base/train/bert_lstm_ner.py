...
class DataProcessor(object):
    ...
    def _read_data(cls, input_file):
        ...
        for line in rf:
            ...
            labels.append(label)
        rf.close()
        print("Nums of sentences: ", len(lines))
        return lines
    ...
    def get_labels(self, labels=None):
        ...
        if os.path.exists(os.path.join(self.output_dir, 'label_list.pkl')):
            ...
            if len(self.labels) > 0:
                ...
                self.labels = ["[PAD]","B-MISC", "I-MISC", "O", "B-PER", "I-PER", "B-ORG", "I-ORG", "B-LOC", "I-LOC", "X","[CLS]","[SEP]"]
        return self.labels'''

        return ['B-MISC',  '[CLS]',  '[SEP]',  'I-LOC',  'B-LOC',  'I-MISC', 'I-PER',  'I-ORG',  'O', 'B-ORG', 'B-PER', 'X']
...
def train(args):
    ...
    if args.do_predict:
        ...
        print("     文本长度： ",len(predict_examples))

        print(id2label)
        print(len(batch_tokens), len(batch_labels))
        # for i, j in zip(batch_tokens, batch_labels):
        #     print(i, " ", j)
        ...
        for i in concat:
            if i==0:
                print("000", batch_tokens[concat.index(i)],"   ",batch_labels[concat.index(i)] )
            if i==12:
                print("012", batch_tokens[concat.index(i)], "   ", batch_labels[concat.index(i)])'''
...
def _write_base(batch_tokens, id2label, prediction, batch_labels, wf, i):
    ...
    if token != "**NULL**" and token != "[CLS]" and token != "[SEP]" and true_l != "X":
        if predict == "X" and not token.startswith("##"):
            predict = "O"
        line = "{}\t{}\t{}\n".format(token, true_l, predict)
        wf.write(line)

# {1: 'B-MISC', 2: '[CLS]', 3: '[SEP]', 4: 'I-LOC', 5: 'B-LOC', 6: 'I-MISC', 7: 'I-PER', 8: 'I-ORG', 9: 'O', 10: 'B-ORG', 11: 'B-PER', 12: 'X'}
